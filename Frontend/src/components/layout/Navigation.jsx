  ...
  return (
    <>
      ...
      <nav className="hidden md:flex h-[74px] bg-white border-b-2 border-gray-900 items-center justify-between px-8">
        ...
        {sidebarPinned ? (
          /* Pinned: show icon buttons inline in the top bar */
          <div className="flex items-center gap-1">
            {menuItems.map(({ icon: Icon, label, tabName }) => (
              <button
                ...
                title={label}
                className={`flex flex-col items-center justify-center gap-0.5 w-14 h-14 border-2 font-black uppercase transition-colors sendstone-sidebar-btn ${
                  activeTab === tabName
                    ? 'sendstone-sidebar-selected bg-blue-500 text-white border-blue-500'
                    : 'sendstone-sidebar-unselected bg-white text-gray-900 border-transparent hover:bg-gray-100'
                }`}
              >
                <Icon className="sendstone-nav-icon" size={20} strokeWidth={2.5} />
                <span className="text-[9px] leading-tight text-center">{label}</span>
              </button>
            ))}
          </div>
        ) : (
          /* Unpinned: show hamburger button */
          <button
      ...
      {/* Desktop Sidebar — only when NOT pinned */}
      {!sidebarPinned && (
        <div
        ...
        >
          <div className="flex flex-col h-full">
            {/* Sidebar Header */}
            <div className="flex items-center justify-between h-[74px] px-4 border-b-2 border-gray-900">
            ...
            <div className="flex-1 flex flex-col p-4 gap-2">
              {menuItems.map(({ icon: Icon, label, tabName }) => (
                <button
                  ...
                  className={`border-2 border-gray-900 font-black uppercase transition-colors flex items-center gap-4 p-4 tracking-wider sendstone-sidebar-btn ${
                    activeTab === tabName
                      ? 'sendstone-sidebar-selected bg-blue-500 text-white'
                      : 'sendstone-sidebar-unselected bg-white text-gray-900 hover:bg-gray-100'
                  }`}
                >
                  <Icon className="sendstone-nav-icon" size={24} strokeWidth={2.5} />
                  <span>{label}</span>
                </button>
              ))}
            </div>
      ...
      {/* Overlay when sidebar is open (unpinned only) */}
